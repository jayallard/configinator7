...
public class MemoryDeployer : IDeployer
{
    ...
    private readonly IDeploymentConfigurationFactory _deploymentConfigurationFactory;

    public MemoryDeployer(IDeploymentConfigurationFactory deploymentConfigurationFactory)
    {
        _deploymentConfigurationFactory = deploymentConfigurationFactory;
    }
    ...
    {
        var configuration = Sanitize(await _deploymentConfigurationFactory.GetConfigurationAsync(request));
        return new DeployerDescription
        {
            ...
            ViewableConfiguration = configuration
        };
    }
    ...
    public async Task<DeployResult> DeployAsync(DeployRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var configuration = await _deploymentConfigurationFactory.GetConfigurationAsync(request);
